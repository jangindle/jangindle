...
<div id=section-estimate class="wrapper">
	<div class="row">
				...
				$(function(){
					...
					$('#estimate-wallpaper-form [name=option-pyeong], #estimate-wallpaper-form .estimate-radio input').blur(function(){
						...
						if( _p == "" ) {
							...
							$( '.estimate-price:eq(0)' ).text("??");
							$( '.estimate-price:eq(1)' ).text("??");
							$( '.estimate-price:eq(2)' ).text("??");
							$( '.estimate-price:eq(3)' ).text("??");
							return ;
						}
						...
						$( '.estimate-price:eq(0)' ).text(_prices[_p_idx[0]]);
						$( '.estimate-price:eq(1)' ).text(_prices[_p_idx[1]]);
						$( '.estimate-price:eq(2)' ).text(_prices[_p_idx[2]]);
						$( '.estimate-price:eq(3)' ).text(_prices[_p_idx[_idx]]);
		...
		<form action="/wallpaper/update_estimate.php" method="post" onsubmit="return simple_ajax_submit( this, function(rt){ if( rt.result == 'success' ){ alert( '요청되었습니다.' ); } } );">
		...
		<input type="hidden" name="type" value="장판"/>
			...
			<form action="./?p=estimate-update" method="post" onsubmit="return simple_ajax_submit( this, function(rt){ if( rt.result == 'success' ){ alert( '요청되었습니다.' ); } } );">
                ...
                <div class="information w-12-4 fl">
                    <div class="im-x-2">
                        <ul class="imb-10">
                            ...
                            <li><label>연락처</label><input type="text" name="tel" placeholder="010-6000-0000" class="submit-required" data-label="고객정보 연락처"/></li>
                            <li><label>시공날짜</label><input type="text" name="c-date"placeholder="2018.02.20" class="datepicker"/></li>
                            <li><label>총 견적</label><input type="text" name="total" placeholder="24만원"/></li>
                        </ul>
                        <label class="estimate-check fa-radio"><i class="fal fa-check-circle"></i> 정보수집에 동의 <input type="checkbox" name="mach" value="풀기계 있음" class="submit-required" data-label="정보수집 동의"></label>
                    </div>
                    <div class="consulting-div">
